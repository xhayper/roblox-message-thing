...
export class MessageServer extends (EventEmitter as new () => TypedEmitter<MessageClientEvents>) {
  ...
  private setupRoutes() {
    ...
    this.app.post(
      ...
      async (request, reply) => {
        ...
        for (const message of messages) {
          if (!client.pendingValidation.includes(message.messageId))
            client.pendingValidation.push(message.messageId);
    ...
    this.app.get("/validation", (request, reply) => {
      const client = this.validateClient(request, reply);
      if (!client) return;

      const pendingValidation = client.pendingValidation;
      client.pendingValidation = [];

      return reply.status(200).send(pendingValidation);
    });
